...
from hclcode import __version__, models, prompts, urls, utils
from hclcode.commands import Commands
from hclcode.history import ChatSummary
from hclcode.io import ConfirmGroup, InputOutput
from hclcode.linter import Linter
from hclcode.llm import litellm
from hclcode.repo import ANY_GIT_ERROR, GitRepo
from hclcode.repomap import RepoMap
from hclcode.run_cmd import run_cmd
from hclcode.sendchat import retry_exceptions, send_completion
from hclcode.utils import format_content, format_messages, format_tokens, is_image_file
...
class Coder:
    ...
    repo = None
    last_hclcode_commit_hash = None
    hclcode_edited_files = None
    ...
    def create(
        ...
        **kwargs,
        ...
        import hclcode.coders as coders
        ...
        if from_coder:
            ...
            update = dict(
                ...
                hclcode_commit_hashes=from_coder.hclcode_commit_hashes,
    ...
    def get_announcements(self):
        lines = []
        lines.append(f"hclcode v{__version__}")
        ...
        if self.repo:
            ...
            if num_files > 1000:
                lines.append(
                    "Warning: For large repos, consider using --subtree-only and .hclcodeignore"
                )
    ...
    def __init__(
        ...
        self.hclcode_commit_hashes = set()
        ...
        if hclcode_commit_hashes:
            self.hclcode_commit_hashes = hclcode_commit_hashes
        else:
            self.hclcode_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} that matches hclcodeignore spec.")
                continue
    ...
    def send_message(self, inp):
        ...
        if edited:
            self.hclcode_edited_files.update(edited)
    ...
    def auto_commit(self, edited, context=None):
        ...
        try:
            res = self.repo.commit(fnames=edited, context=context, hclcode_edits=True)
            if res:
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_hclcode_commit_hash = commit_hash
        self.hclcode_commit_hashes.add(commit_hash)
        self.last_hclcode_commit_message = commit_message
        if self.show_diffs:
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head_commit_sha():
            self.io.tool_output("You can use /undo to undo and discard each hclcode commit.")

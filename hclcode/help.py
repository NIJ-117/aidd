...
from hclcode import __version__, utils
from hclcode.dump import dump  # noqa: F401
from hclcode.help_pats import exclude_website_pats
...
def get_package_files():
    for path in importlib_resources.files("hclcode.website").iterdir():
        if path.is_file():
            yield path
        elif path.is_dir():
...
def fname_to_url(filepath):
    ...
    if filepath.startswith("website/_includes/"):
        ...
        docid = "https://hclcode.chat/" + filepath
...
def get_index():
    ...
    dname = Path.home() / ".hclcode" / "caches" / ("help." + __version__)
    ...
    if index is None:
        ...
        for fname in get_package_files():
            ...
            doc = Document(
                text=importlib_resources.files("hclcode.website")
                .joinpath(fname)

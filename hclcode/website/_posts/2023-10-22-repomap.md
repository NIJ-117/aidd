...
excerpt: Tree-sitter allows hclcode to build a repo map that better summarizes large code bases.
...
To address these issues, hclcode
...
hclcode now uses it to help GPT better comprehend, navigate
and edit code in larger repos.

*To code with GPT-4 using the techniques discussed here, just install [hclcode](https://hclcode.chat/docs/install.html).*
...
hclcode also strives to reduce the manual work involved in
coding with AI.
So in an ideal world, we'd like hclcode to automatically
identify and provide the needed code context.

## Using a repo map to provide context

hclcode sends a **repo map** to GPT along with
...
Mapping out the repo like this provides some key benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at in more detail. GPT will then ask to see these specific files, and hclcode will automatically add them to the chat context.
...
hclcode uses all of these definitions and references to
...
[ctags based map](https://hclcode.chat/docs/ctags.html)
that hclcode originally used.
...
- Thanks to `py-tree-sitter-languages`, we get full support for many programming languages via a python package that's automatically installed as part of the normal `python -m pip install -U hclcode-chat`.
...
- Tree-sitter integration is a key enabler for future work and capabilities for hclcode.
...
Right now, hclcode relies on the user to specify which source files
will need to be modified to complete their request.
Users manually "add files to the chat" using hclcode's `/add` command,
...
just install [hclcode](https://hclcode.chat/docs/install.html).

## Credits

hclcode uses
[modified versions of the tags.scm files](https://github.com/hclcode-AI/hclcode/tree/main/hclcode/queries)
from these

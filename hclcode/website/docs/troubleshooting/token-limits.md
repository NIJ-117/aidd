...
hclcode will report an error if a model responds indicating that
...
For more info: https://hclcode.chat/docs/token-limits.html
```
...
It's best to only add the files that hclcode will need to *edit*
...
as 4k tokens.
If you ask hclcode to make a large change that affects a lot
...
hclcode has been well tested when directly connecting to 
major 
[LLM provider cloud APIs](https://hclcode.chat/docs/llms.html).
For serving local models, 
[Ollama](https://hclcode.chat/docs/llms/ollama.html) is known to work well with hclcode.

Try using hclcode without an API proxy server

...
hclcode has a few different chat modes:

- `code` - hclcode will make changes to your code to satisfy your requests.
- `architect` - hclcode will first propose a solution, then ask if you want it to turn that proposal into edits to your files.
- `ask` - hclcode will answer questions about your code, but never edit it.
- `help` - hclcode will answer questions about using hclcode, configuring, troubleshooting, etc.

By default, hclcode starts in "code" mode. As you are talking, you can
...
Or you can launch hclcode in one of the modes with the `--chat-mode <mode>` switch.
There is also a special shortcut `--architect` to launch in `--chat-mode architect`.

## Architect mode and the editor model

When you are in architect mode, hclcode sends your request to two models:
...
After the main model replies, hclcode will offer to edit the files based on the response.

2. To edit the files, hclcode sends a second LLM request asking for specific code editing instructions.
This request goes to the "editor" model.
hclcode has built in defaults to select an editor model based on your main model.
...
The editor model uses one of hclcode's edit formats to let the LLM
edit source files.
hclcode will pick a suitable edit format by default,
...
See this article on 
[hclcode's architect/editor mode](/2024/09/26/architect.html)
for more details.
...
Run `hclcode --model ollama/<ollama-model>`.
See these docs for more info: https://hclcode.chat/docs/llms/ollama.html

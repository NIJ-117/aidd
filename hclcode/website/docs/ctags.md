...
hclcode no longer uses ctags to build a repo map.
Please see the newer article about
[using tree-sitter to build a better repo map](https://hclcode.chat/docs/repomap.html).
...
To address these issues, `hclcode` now
...
[chat transcript](https://hclcode.chat/examples/add-test.html)
...
To code with GPT-4 using the techniques discussed here:
  ...
  - Install [hclcode](https://hclcode.chat/docs/install.html).
  - Install universal ctags.
  - Run `hclcode` inside your repo, and it should say "Repo-map: universal-ctags using 1024 tokens".
...
This works pretty well, and is supported by `hclcode` -- you
...
The latest version of `hclcode` sends a **repo map** to GPT along with
...
Here's a
sample of the map of the hclcode repo, just showing the maps of
[main.py](https://github.com/hclcode-AI/hclcode/blob/main/hclcode/main.py)
and
[io.py](https://github.com/hclcode-AI/hclcode/blob/main/hclcode/io.py)
:
...
Mapping out the repo like this provides some benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at. GPT will then ask to see these specific files, and `hclcode` will automatically add them to the chat context (with user approval).
...
Sending an appropriate subset of the repo map would help `hclcode` work

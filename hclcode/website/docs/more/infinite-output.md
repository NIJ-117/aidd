...
parent: More info
nav_order: 920
description: hclcode can handle "infinite output" from models that support prefill.
---
...
hclcode is able to work around this limit with models that support
"prefilling" the assistant response.
When you use hclcode with a model that supports prefill, you will see
...
When hclcode is collecting code edits from a model and
it hits the output token limit,
hclcode simply initiates another LLM request with the partial
response prefilled.
...
hclcode supports "infinite output" for models that support "prefill",
such as:

...
In most cases, you can safely ignore hclcode's warning about unknown context
window size and model costs.

But, you can register context window limits and costs for models that aren't known
to hclcode. Create a `.hclcode.model.metadata.json` file in one of these locations:

- Your home directory.
- The root if your git repo.
- The current directory where you launch hclcode.
...
in the `.hclcode.model.metadata.json` file.
For example, use `deepseek/deepseek-chat`, not just `deepseek-chat`.

## Model settings

hclcode has a number of settings that control how it works with
different models.
...
a model that hclcode doesn't know about.

To do that,
create a `.hclcode.model.settings.yml` file in one of these locations:
...
[models.py](https://github.com/hclcode-AI/hclcode/blob/main/hclcode/models.py)
file for more details about all of the model setting that hclcode supports.

<!--[[[cog
from hclcode.models import get_model_settings_as_yaml
cog.out("```yaml\n")

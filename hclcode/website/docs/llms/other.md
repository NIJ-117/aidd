...
hclcode uses the [litellm](https://docs.litellm.ai/docs/providers) package
to connect to hundreds of other models.
You can use `hclcode --model <model-name>` to use any supported model.

To explore the list of supported models you can run `hclcode --list-models <model-name>`
with a partial model name.
If the supplied name is not an exact match for a known model, hclcode will
return a list of possible matching models.
For example:

```
$ hclcode --list-models turbo
...
hclcode uses the LiteLLM package to connect to LLM providers.

...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var blameData = {
        labels: labels,
        datasets: [{
            label: 'hclcode\'s percent of new code by release',
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.hclcode_percentage }}, lines: {{ row.hclcode_total }} },{% endfor %}],
    ...
    var linesData = {
        labels: labels,
        datasets: [{
            label: 'hclcode\'s lines of new code',
            data: [{% for row in site.data.blame %}{ x: '{{ row.end_tag }}', y: {{ row.hclcode_total }} },{% endfor %}],
    ...
    var blameChart = new Chart(blameCtx, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            var label = 'hclcode\'s contribution';
                ...
                title: {
                    display: true,
                    text: 'Percent of new code written by hclcode, by release',
                    font: {
                        size: 16
                    }
                }
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            var label = 'New lines of code by hclcode';
                            var value = context.parsed.y || 0;
                            return `${label}: ${value}`;
                        }
                    }
                ...
                title: {
                    display: true,
                    text: 'Lines of new code written by hclcode, by release',
                    font: {
                        size: 16
                    }
                }

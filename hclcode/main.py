...
from hclcode import __version__, models, urls, utils
from hclcode.args import get_parser
from hclcode.coders import Coder
from hclcode.commands import Commands, SwitchCoder
from hclcode.format_settings import format_settings, scrub_sensitive_info
from hclcode.history import ChatSummary
from hclcode.io import InputOutput
from hclcode.llm import litellm  # noqa: F401; properly init litellm on launch
from hclcode.repo import ANY_GIT_ERROR, GitRepo
from hclcode.report import report_uncaught_exceptions
from hclcode.versioncheck import check_version, install_from_main_branch, install_upgrade
...
def setup_git(git_root, io):
    ...
    if git_root:
        ...
        io.tool_warning("You should probably run hclcode in a directory, not your home dir.")
        return
    elif io.confirm_ask("No git repo found, create one to track hclcode's changes (recommended)?"):
...
def check_gitignore(git_root, io, ask=True):
    ...
    try:
        repo = git.Repo(git_root)
        if repo.ignored(".hclcode") and repo.ignored(".env"):
            return
    except ANY_GIT_ERROR:
        pass

    patterns = [".hclcode*", ".env"]
    patterns_to_add = []
...
def check_streamlit_install(io):
    return utils.check_pip_install_extra(
        io,
        "streamlit",
        "You need to install the hclcode browser feature",
...
def register_models(git_root, model_settings_fname, io, verbose=False):
    model_settings_files = generate_search_path_list(
        ".hclcode.model.settings.yml", git_root, model_settings_fname
    )
    ...
    try:
        ...
        io.tool_error(f"Error loading hclcode model settings: {e}")
        return 1
...
def register_litellm_models(git_root, model_metadata_fname, io, verbose=False):
    model_metatdata_files = generate_search_path_list(
        ".hclcode.model.metadata.json", git_root, model_metadata_fname
    )
...
def sanity_check_repo(repo, io):
    ...
    if bad_ver:
        io.tool_error("hclcode only works with git repos with version number 1 or 2.")
        io.tool_output("You may be able to convert your repo: git update-index --index-version=2")
        io.tool_output("Or run hclcode --no-git to proceed without using git.")
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    conf_fname = Path(".hclcode.conf.yml")
    ...
    io.tool_output('Use /help <question> for help, run "hclcode --help" to see cmd line args')
...
def check_and_load_imports(io, verbose=False):
    installs_file = Path.home() / ".hclcode" / "installs.json"
    ...
    try:
        ...
        if str(key) not in installs:
            ...
            try:
                ...
                io.tool_error(str(err))
                io.tool_output("Error loading required imports. Did you install hclcode properly?")
                io.tool_output("https://hclcode.chat/docs/install/install.html")
                sys.exit(1)

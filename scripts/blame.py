    ...
    for file in files:
        ...
        if file_counts:
            ...
            for author, count in file_counts.items():
                ...
                if "(hclcode)" in author.lower():
                    hclcode_total += count

    total_lines = sum(grand_total.values())
    hclcode_percentage = (hclcode_total / total_lines) * 100 if total_lines > 0 else 0

    end_date = get_tag_date(end_tag if end_tag else "HEAD")

    return all_file_counts, grand_total, total_lines, hclcode_total, hclcode_percentage, end_date
...
def get_commit_authors(commits):
    ...
    for commit in commits:
        ...
        if commit_message.lower().startswith("hclcode:"):
            author += " (hclcode)"
...
def process_all_tags_since(start_tag):
    ...
    for i in tqdm(range(len(tags) - 1), desc="Processing tags"):
        ...
        all_file_counts, grand_total, total_lines, hclcode_total, hclcode_percentage, end_date = blame(
            start_tag, end_tag
        )
        results.append(
            {
                ...
                "hclcode_total": hclcode_total,
                "hclcode_percentage": round(hclcode_percentage, 2),
            }
        )
    return results
...
def main():
    parser = argparse.ArgumentParser(description="Get hclcode/non-hclcode blame stats")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Find all tags since the specified tag and print hclcode percentage between each pair of"
            " successive tags"
        ),
    )
    parser.add_argument(
    ...
    if not args.all_since:
        print(f"- hclcode wrote {round(hclcode_percentage)}% of the code in this release.")

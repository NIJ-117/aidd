...
def main(
    ...
    replay: str = typer.Option(
        None,
        "--replay",
        help="Replay previous .hclcode.chat.history.md responses from previous benchmark run",
    ),
    ...
    no_hclcode: bool = typer.Option(False, "--no-hclcode", help="Do not run hclcode"),
    ...
    if "hclcode_DOCKER" not in os.environ:
...
def show_diffs(dirnames):
    ...
    for testcase in testcases:
        ...
        for outcome, dirname in zip(all_outcomes, dirnames):
            print(outcome, f"{dirname}/{testcase}/.hclcode.chat.history.md")
...
def load_results(dirname):
    dirname = Path(dirname)
    all_results = [json.loads(fname.read_text()) for fname in dirname.glob("*/.hclcode.results.json")]
    return all_results
...
def summarize_results(dirname):
    ...
    command = f"hclcode --model {a_model}"
    ...
    for hsh in commit_hashes:
        ...
        try:
            version = subprocess.check_output(
                ["git", "show", f"{hsh}:hclcode/__init__.py"], universal_newlines=True
            )
...
def get_replayed_content(replay_dname, test_dname):
    ...
    replay_fname = replay_dname / test_name / ".hclcode.chat.history.md"
    dump(replay_fname)
...
def run_test(original_dname, testdir, *args, **kwargs):
    try:
        ...
        results_fname = testdir / ".hclcode.results.json"
...
def run_test_real(
    ...
    history_fname = testdir / ".hclcode.chat.history.md"

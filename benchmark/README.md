...
# hclcode code editing benchmark harness

hclcode uses a "code editing" benchmark to quantitatively measure how well it works
...
See [this writeup for a longer discussion about the benchmark and how to interpret the results](https://hclcode.chat/docs/benchmarks.html).
...
There are 3 main tasks involved in benchmarking hclcode:
...
```
# Clone the hclcode repo
git clone git@github.com:hclcode-AI/hclcode.git

# Create the scratch dir to hold benchmarking results inside the main hclcode dir:
cd hclcode
...
# Inside the container, install hclcode as a development build.
...
- `--model` is the name of the model, same as you would pass directly to `hclcode`.
- `--edit-format` is the name of the edit format, same as you would pass directly to `hclcode`. When working with an experimental LLM, I recommend starting with `whole`
...
[hclcode leaderboard data files](https://github.com/hclcode-AI/hclcode/blob/main/hclcode/website/_data/).
...
[hclcode leaderboard data files](https://github.com/hclcode-AI/hclcode/blob/main/hclcode/website/_data/).
- These scripts are not intended for use by typical hclcode end users.

...
from hclcode.coders.udiff_coder import find_diffs
...
1. Add the `--check-update` argument to the argument parser in `hclcode/main.py`.
2. Modify the `check_version` function in `hclcode/versioncheck.py` to return a boolean indicating whether an update is available.
3. Use the returned value from `check_version` in `hclcode/main.py` to set the exit status code when `--check-update` is used.

Here are the diffs for those changes:

```diff
--- hclcode/versioncheck.py
+++ hclcode/versioncheck.py
@@ ... @@

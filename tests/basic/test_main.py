...
from hclcode.dump import dump  # noqa: F401
from hclcode.io import InputOutput
from hclcode.main import check_gitignore, main, setup_git
from hclcode.utils import GitTemporaryDirectory, IgnorantTemporaryDirectory, make_repo
...
class TestMain(TestCase):
    def setUp(self):
        ...
        os.environ["hclcode_CHECK_UPDATE"] = "false"
        self.original_cwd = os.getcwd()
        self.tempdir_obj = IgnorantTemporaryDirectory()
        self.tempdir = self.tempdir_obj.name
        os.chdir(self.tempdir)
        # Fake home directory prevents tests from using the real ~/.hclcode.conf.yml file:
    ...
    @patch("hclcode.repo.GitRepo.get_commit_message", return_value="mock commit message")
    ...
    def test_main_with_git_config_yml(self):
        ...
        Path(".hclcode.conf.yml").write_text("auto-commits: false\n")
        with patch("hclcode.coders.Coder.create") as MockCoder:
            main(["--yes"], input=DummyInput(), output=DummyOutput())
            _, kwargs = MockCoder.call_args
            assert kwargs["auto_commits"] is False

        Path(".hclcode.conf.yml").write_text("auto-commits: true\n")
        with patch("hclcode.coders.Coder.create") as MockCoder:
    ...
    def test_main_with_empty_git_dir_new_subdir_file(self):
        ...
        # Because hclcode will try and `git add` a file that's already in the repo.
    ...
    def test_setup_git(self):
        ...
        self.assertEqual(".hclcode*", gitignore.read_text().splitlines()[0])
    ...
    def test_encodings_arg(self):
        ...
        with GitTemporaryDirectory():
            with patch("hclcode.coders.Coder.create") as MockCoder:  # noqa: F841
                with patch("hclcode.main.InputOutput") as MockSend:
    ...
    def test_main_exit_calls_version_check(self):
        with GitTemporaryDirectory():
            with (
                patch("hclcode.main.check_version") as mock_check_version,
                patch("hclcode.main.InputOutput") as mock_input_output,
            ):
                main(["--exit", "--check-update"], input=DummyInput(), output=DummyOutput())
                mock_check_version.assert_called_once()
                mock_input_output.assert_called_once()

    @patch("hclcode.main.InputOutput")
    @patch("hclcode.coders.base_coder.Coder.run")
    ...
    def test_dark_mode_sets_code_theme(self):
        ...
        with patch("hclcode.main.InputOutput") as MockInputOutput:
    ...
    def test_env_file_flag_sets_automatic_variable(self):
        env_file_path = self.create_env_file(".env.test", "hclcode_DARK_MODE=True")
        with patch("hclcode.main.InputOutput") as MockInputOutput:
            MockInputOutput.return_value.get_input.return_value = None
    ...
    def test_default_env_file_sets_automatic_variable(self):
        self.create_env_file(".env", "hclcode_DARK_MODE=True")
        with patch("hclcode.main.InputOutput") as MockInputOutput:
            MockInputOutput.return_value.get_input.return_value = None
            MockInputOutput.return_value.get_input.confirm_ask = True
            main(["--no-git", "--exit"], input=DummyInput(), output=DummyOutput())
    ...
    def test_false_vals_in_env_file(self):
        self.create_env_file(".env", "hclcode_SHOW_DIFFS=off")
        with patch("hclcode.coders.Coder.create") as MockCoder:
    ...
    def test_true_vals_in_env_file(self):
        self.create_env_file(".env", "hclcode_SHOW_DIFFS=on")
        with patch("hclcode.coders.Coder.create") as MockCoder:
            main(["--no-git"], input=DummyInput(), output=DummyOutput())
            MockCoder.assert_called_once()
            _, kwargs = MockCoder.call_args
    ...
    def test_lint_option(self):
        with GitTemporaryDirectory() as git_dir:
            ...
            with patch("hclcode.linter.Linter.lint") as MockLinter:
    ...
    def test_verbose_mode_lists_env_vars(self):
        self.create_env_file(".env", "hclcode_DARK_MODE=on")
        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            ...
            relevant_output = "\n".join(
                ...
                if "hclcode_DARK_MODE" in line or "dark_mode" in line
            )  # this bit just helps failing assertions to be easier to read
            self.assertIn("hclcode_DARK_MODE", relevant_output)
            self.assertIn("dark_mode", relevant_output)
            self.assertRegex(relevant_output, r"hclcode_DARK_MODE:\s+on")
    ...
    def test_yaml_config_file_loading(self):
        with GitTemporaryDirectory() as git_dir:
            ...
            # Create .hclcode.conf.yml files in different locations
            home_config = fake_home / ".hclcode.conf.yml"
            git_config = git_dir / ".hclcode.conf.yml"
            cwd_config = cwd / ".hclcode.conf.yml"
            named_config = git_dir / "named.hclcode.conf.yml"
            ...
            with (
                ...
                patch("hclcode.coders.Coder.create") as MockCoder,
            ):
    ...
    def test_map_tokens_option(self):
        with GitTemporaryDirectory():
            with patch("hclcode.coders.base_coder.RepoMap") as MockRepoMap:
    ...
    def test_model_metadata_file(self):
        with GitTemporaryDirectory():
            metadata_file = Path(".hclcode.model.metadata.json")
